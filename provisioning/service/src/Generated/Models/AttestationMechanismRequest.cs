// <auto-generated>
// Code generated by Microsoft (R) AutoRest Code Generator.
// Changes may cause incorrect behavior and will be lost if the code is
// regenerated.
// </auto-generated>

namespace Microsoft.Azure.Devices.Provisioning.Service.Models
{
    using Microsoft.Rest;
    using Newtonsoft.Json;
    using System.Linq;

    /// <summary>
    /// Attestation mechanism for individualEnrollment as well as
    /// enrollmentGroup
    /// </summary>
    public partial class AttestationMechanismRequest
    {
        /// <summary>
        /// Initializes a new instance of the AttestationMechanismRequest
        /// class.
        /// </summary>
        public AttestationMechanismRequest()
        {
            CustomInit();
        }

        /// <summary>
        /// Initializes a new instance of the AttestationMechanismRequest
        /// class.
        /// </summary>
        /// <param name="type">Attestation Type. Possible values include:
        /// 'none', 'tpm', 'x509', 'symmetricKey'</param>
        /// <param name="tpm">TPM attestation method.</param>
        /// <param name="x509">X509 attestation method.</param>
        /// <param name="symmetricKey">Symmetric Key attestation
        /// method.</param>
        public AttestationMechanismRequest(string type, TpmAttestation tpm = default(TpmAttestation), X509AttestationRequest x509 = default(X509AttestationRequest), SymmetricKeyAttestation symmetricKey = default(SymmetricKeyAttestation))
        {
            Type = type;
            Tpm = tpm;
            X509 = x509;
            SymmetricKey = symmetricKey;
            CustomInit();
        }

        /// <summary>
        /// An initialization method that performs custom operations like setting defaults
        /// </summary>
        partial void CustomInit();

        /// <summary>
        /// Gets or sets attestation Type. Possible values include: 'none',
        /// 'tpm', 'x509', 'symmetricKey'
        /// </summary>
        [JsonProperty(PropertyName = "type")]
        public string Type { get; set; }

        /// <summary>
        /// Gets or sets TPM attestation method.
        /// </summary>
        [JsonProperty(PropertyName = "tpm")]
        public TpmAttestation Tpm { get; set; }

        /// <summary>
        /// Gets or sets X509 attestation method.
        /// </summary>
        [JsonProperty(PropertyName = "x509")]
        public X509AttestationRequest X509 { get; set; }

        /// <summary>
        /// Gets or sets symmetric Key attestation method.
        /// </summary>
        [JsonProperty(PropertyName = "symmetricKey")]
        public SymmetricKeyAttestation SymmetricKey { get; set; }

        /// <summary>
        /// Validate the object.
        /// </summary>
        /// <exception cref="ValidationException">
        /// Thrown if validation fails
        /// </exception>
        public virtual void Validate()
        {
            if (Type == null)
            {
                throw new ValidationException(ValidationRules.CannotBeNull, "Type");
            }
            if (Tpm != null)
            {
                Tpm.Validate();
            }
        }
    }
}
